...
from Cover_strategy import stopOut
from balance_strategy import balancestrategy_begin, cancel_balancestrategy_orders, clear_tradecoin_remains, \
    get_total_fee
from crash_callback import clear_remain
from grid_strategy import gridbegin, cancelgridorders, clear_grid_remain, gridbegin2
from loggerConfig import logger
from market_maker.handle import get_grid_orderlist, synchronize_grid_orderlist
from order_copy import sum_profit
from strategy_copy.balance_copy import balance_strategy_copy
from strategy_copy.grid_copy import grid_stop
from tools.Config import gap_amount, amountlimit, Cancel_url, Remain_url
from tools.databasePool import r1, r2, r4, POOL_grid, POOL
    ...
    if status == 1:  # 开启
        ...
        try:
            if directionType == 0:
                ...
                profitRate = profit / existingUsdt  # 盈利率
                    ...
                    {"success": "true",
                     "data": {"msg": "用户{}子账户{}停止动态平衡策略{}".format(userUuid, apiAccountId, strategyId), "code": 1,
                              "data": {"strategyId": strategyId, "status": status, "userUuid": userUuid,
                                       ...
                                       "totalValcoinFee": totalValcoinFee, "profit": profit, "profitRate": profitRate,
...
def runCopy():
    ...
    strategyname_dict = {1: '网格策略', 4: '智能追踪策略', 10: '动态平衡策略', 13: '补仓策略'}
...
def runStrategy():
    ...
    if status == 1:
        if strategyType == 14:
            r2.hset('Crash_strategy', strategyId, json.dumps(strategydata))
            print("用户{}子账户{}开启暴跌反弹策略,策略ID{}".format(userUuid, apiAccountId, strategyId))
            return jsonify({"success": "true",
                            "data": {
                                "msg": "用户{}子账户{}开启暴跌反弹策略{}".format(userUuid, apiAccountId, strategyId),
                                "code": 1, "data": {"strategyId": strategyId, 'status': status}
