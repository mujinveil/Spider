...
class MongoRedisUrlManager:
    ...
    def __init__(self, server_ip='localhost', client=None, expires=timedelta(days=30)):
        ...
        if self.db.lagou.count() is 0:
            self.db.lagou.create_index('status')

    def dequeueUrl(self):
        record = self.db.lagou.find_one_and_update(
            { 'status': 'new'}, 
    ...
    def finishUrl(self, url):
        ...
        self.db.lagou.update({'_id': hashlib.md5(url.encode('utf8')).hexdigest()}, {'$set': record}, upsert=False)
