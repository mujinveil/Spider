...
connection_pool = ConnectionPool(host='localhost', port=6379, db=8)
...
class JDCommentSpider(object):
    ...
    def parse_comment(self, data):
        ...
        comment_item = {}
        if response != '':
            response = response.replace(r"fetchJSON_comment98vv118416(", "")
            response = response.replace(r");", "")
            content = json.loads(response)
            max_page = content.get("maxPage")
            ...
            for comment in comments:
                ...
                logging.info('商品评论插入成功,sku_id:{}'.format(sku_id))
            ...
            if page >= max_page:
                summary_item = {}
                comment_summary = content.get("productCommentSummary")
                summary_item['_id'] = ObjectId()
                ...
                logging.info('商品评论汇总插入成功,sku_id:{}'.format(sku_id))

    def run(self):
        while True:
            try:
                data = redis_conn.rpop('jd_comment')
                if len(data) == 0:
                    return
