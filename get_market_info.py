...
def get_currentprice0(platform, symbol):
    try:
        if platform == "zb":
            ...
            params = {'symbol': symbol.replace("_", "").lower()}
            # try:
            #     res1 = requests.get('https://api.huobi.pro/market/detail', params=params, timeout=2)
            #     dict1 = json.loads(res1.content.decode())
            #     currentprice = dict1["tick"]['close']
            # except:
            #     currentprice = 0
            try:
                res1 = requests.get(
        ...
        print("获取{}现价失败，报错信息{}".format(platform, e))
        return 0
