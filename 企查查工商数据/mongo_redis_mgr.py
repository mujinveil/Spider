...
class MongoRedisUrlManager:
    ...
    def __init__(self, server_ip='localhost', client=None, expires=timedelta(days=30)):
        ...
        self.client = MongoClient('192.168.18.180', 27017) if client is None else client
        ...
        if self.db.qcc.count() is 0:
            self.db.qcc.create_index('status')

    def dequeueUrl(self):
        record = self.db.qcc.find_one_and_update(
            { 'status': 'new'}, 
    ...
    def finishUrl(self, url):
        ...
        self.db.qcc.update({'_id': hashlib.md5(url.encode('utf8')).hexdigest()}, {'$set': record}, upsert=False)
